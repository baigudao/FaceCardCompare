...
import com.taisau.facecardcompare.util.IndexAdapter;
import com.taisau.facecardcompare.util.Indexable;
...
public class ZSideBar extends View
{
  ...
  public SimpleArrayMap<Integer, String> getIndexMap() {
    return indexMap;
  }
  ...
  protected void onDraw(Canvas canvas)
  {
    ...
    for (int i = 0; i < indexMap.size(); i++) {
      ...
      int colorId = i == choose ? R.color.color_34c7ba : R.color.white_fff;
  ...
  private void initIndex(@NonNull RecyclerView.Adapter adapter)
  {
    ...
    for (int i = 0; i < adapter.getItemCount(); i++) {
      Indexable item = (Indexable) ((IndexAdapter) adapter).getItem(i);
      if (i == 0) {
        indexMap.put(i, item.getIndex());
      } else {
        Indexable preItem = (Indexable) ((IndexAdapter) adapter).getItem(i - 1);
