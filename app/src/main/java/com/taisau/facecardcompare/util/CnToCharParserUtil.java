...
/**
 * Created by ds  on 2016/12/15 10:48
 */

public class CnToCharParserUtil {
    private static Map<String, Integer> spellMap = null;
    private static Map<Character, String> uncommonWordsMap = null;
    private static CnToCharParserUtil cnParser = new CnToCharParserUtil();
    ...
    private CnToCharParserUtil() {

    }

    public static CnToCharParserUtil getInstance() {
        return cnParser;
    }
        ...
        for (int i = 0, len = chars.length; i < len; i++) {
            ...
            if (ascii == 0) {
                ...
                if (spell == null || spell.length() == 0) {
                    ...
                    if (onlyFirstSpell) {
                        sb.append((spell.startsWith("'") && spell.length() > 1) ? spell.substring(1, 2)
                                : spell.substring(0, 1));
                    } else {
                        sb.append(spell);
                    }
